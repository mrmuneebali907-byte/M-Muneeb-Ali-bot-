...
module.exports = {
  ...
  async execute(sock, msg, args, extra) {
    try {
      ...
      if (!mediaData.length) {
        try {
          const r = await axios.get(`https://api.siputzx.my.id/api/d/igdl?url=${encodeURIComponent(text)}`, { timeout: 20000 });
          const d = r.data?.data;
          if (Array.isArray(d) && d.length) {
            mediaData = d.map(item => ({ url: item.url || item.download, type: item.type }));
          }
        } catch {}
      }

      // Fallback 2: snapinsta API
      if (!mediaData.length) {
        try {
          const r = await axios.get(`https://snapinsta.app/api/convert?url=${encodeURIComponent(text)}`, {
            timeout: 20000,
            headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)' }
          });
          if (r.data?.url) mediaData = [{ url: r.data.url, type: 'image' }];
          else if (Array.isArray(r.data?.images)) {
            mediaData = r.data.images.map(u => ({ url: u, type: 'image' }));
          }
        } catch {}
      }

      // Fallback 3: publically shared API
      if (!mediaData.length) {
        try {
          const r = await axios.get(`https://api.tiklydown.eu.org/api/download/v2?url=${encodeURIComponent(text)}`, { timeout: 20000 });
          const d = r.data?.data;
          if (d?.images?.length) mediaData = d.images.map(u => ({ url: u, type: 'image' }));
          else if (d?.play) mediaData = [{ url: d.play, type: 'video' }];
        } catch {}
      }
