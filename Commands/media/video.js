...
module.exports = {
  ...
  async execute(sock, msg, args) {
    try {
      ...
      // Get video: try 8 sources with fallback chain
      let videoData;
      const videoSources = APIs.getYTVideoSources(videoUrl);
      let found = false;
      for (const source of videoSources) {
        try {
          videoData = await source();
          if (videoData?.download) { found = true; break; }
        } catch {}
      }
      if (!found) {
        // Last resort: legacy methods
        try { videoData = await APIs.getEliteProTechVideoByUrl(videoUrl); }
        catch { try { videoData = await APIs.getYupraVideoByUrl(videoUrl); }
          catch { videoData = await APIs.getOkatsuVideoByUrl(videoUrl); } }
      }
