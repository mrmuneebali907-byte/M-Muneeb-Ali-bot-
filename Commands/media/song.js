...
module.exports = {
  ...
  async execute(sock, msg, args) {
    try {
      ...
      // Try multiple APIs with fallback chain (8+ sources)
      let audioData;
      let audioBuffer;
      let downloadSuccess = false;

      const apiMethods = [
        { name: 'Siputzx',     method: () => APIs.getYTAudioSources(video.url)[0]() },
        { name: 'Vreden',      method: () => APIs.getYTAudioSources(video.url)[1]() },
        { name: 'Nekosite',    method: () => APIs.getYTAudioSources(video.url)[2]() },
        { name: 'Cobalt',      method: () => APIs.getYTAudioSources(video.url)[3]() },
        ...
        { name: 'Bk9',         method: () => APIs.getYTAudioSources(video.url)[5]() },
        { name: 'Agcresi',     method: () => APIs.getYTAudioSources(video.url)[6]() }
      ];
